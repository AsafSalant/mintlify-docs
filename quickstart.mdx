---
title: "Quickstart"
description: "Get started with Olyptik in minutes"
---

## Introduction

Olyptik is a powerful web scraping and data extraction platform that helps you gather and process web data efficiently. This guide will walk you through the essential steps to get started with our API.

## Getting Started with the API

### Authentication

First, you'll need an API key. You can get one from your [settings page](https://app.olyptik.io/0/settings/crawl). Include it in your requests using the Authorization header:

```bash
Authorization: Bearer your-api-key
```

### Step 1: Create a Crawl

Start by creating a crawl with your target URL. This initiates the web scraping process:

<CodeGroup>

```javascript javascript
// npm i olyptik
import Olyptik from "olyptik";

const olyptik = new Olyptik({ apiKey: 'your-api-key' });

const crawl = await olyptik.createCrawl({
  startUrl: 'https://example.com',
  maxResults: 50
});
  
console.log('Crawl created:', crawl.id);
```

```bash curl
curl -X POST https://api.olyptik.io/v1/crawls \
  -H "Authorization: Bearer your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "startUrl": "https://example.com",
    "maxResults": 50
  }'
```

</CodeGroup>

The response will include a `crawl` object that you'll use to track and retrieve results:

```json Crawl object
{
  "id": "crawl_123",
  "status": "running",
  "startUrls": ["https://example.com"],
  "maxDepth": 5,
  "maxResults": 50,
  "numberOfResults": 0,
  "includeLinks": true,
  "origin": "api",
  "timeout": 60,
  "durationInSeconds": 0,
  "completedAt": null,
  "createdAt": "2024-03-21T10:00:00Z",
  "updatedAt": "2024-03-21T10:00:00Z"
}
```

### Step 2: Monitor Crawl Status

You have two options to track your crawl's progress:

#### Option 1: Webhook Notifications (Recommended)

Configure a webhook URL in your [settings page](https://app.olyptik.io/0/settings/crawl) to receive real-time status updates. You can choose from two notification types:

- **Crawl Status Changes**: Receive notifications when crawl status changes (succeeded, failed, timed out, etc.)
- **Individual Results**: Receive notifications for each page crawled during the process

You can enable either option or both simultaneously.

<CodeGroup>
```json Crawl status change event
{
  "secret": "u9dv22c5",
  "eventType": "crawl_status_change",
  "data": {
    "crawl": {
      "id": "6870336416bf934c31bffdd8",
      "status": "timed_out",
      "durationInSeconds": 45001,
      "brandId": "680e8bc08cdb14f10180db6a",
      "startUrls": ["https://example.com"],
      "numberOfResults": 19,
      "useSitemap": false,
      "maxDepth": 5,
      "maxResults": 20,
      "includeLinks": false,
      "origin": "api",
      "timeout": 60,
      "createdAt": "2025-07-10T21:40:52.580Z",
      "updatedAt": "2025-07-11T10:10:54.251Z",
      "completedAt": "2025-07-11T10:10:54.250Z"
    }
  }
}
```
```json Crawl result created event
{
  "secret": "u9dv22c5",
  "eventType": "crawl_result_created",
  "data": {
    "crawlResult": {
      "id": "6870e3c687c81925622df89e"
      "crawlId": "6870e36787c81925622df818",
      "brandId": "680e8bc08cdb14f10180db6a",
      "url": "https://example.com/solutions/network-observability",
      "depthOfUrl": 4,
      "title": "Kubernetes Network Observability",
      "markdown": "## Awesome website content...",
      "createdAt": "2025-07-11T10:13:26.966Z",
      "updatedAt": "2025-07-11T10:13:26.966Z",
    }
  }
}
```
</CodeGroup>

#### Option 2: Polling (Alternative)
If you haven't configured webhook notifications, you can poll the crawl status:

<CodeGroup>

```javascript javascript
async function getCrawlStatus(crawlId) {
  const status = await olyptik.getCrawl(crawlId);
  console.log('Crawl status:', status);
  return status;
}
```

```bash curl
curl https://api.olyptik.io/v1/crawls/{crawl_id} \
  -H "Authorization: Bearer your-api-key"
```

</CodeGroup>
### Step 3: Get Crawl Results

Once the crawl is complete, retrieve the results:

<CodeGroup>

```javascript javascript
const page = 1;
const itemsInPage = 50;
const results = await olyptik.getCrawlResults(crawlId, page, itemsInPage);
console.log(results);
```

```bash curl
curl https://api.olyptik.io/v1/crawls/{crawl_id}/results?page=1&limit=50 \
  -H "Authorization: Bearer your-api-key"
```

</CodeGroup>

Example response:
```json
{
  "page": 1,
  "limit": 10,
  "totalPages": 2,
  "totalResults": 19,
  "results": [
    {
      "id": "6870e3c687c81925622df89e",
      "crawlId": "6870e36787c81925622df818",
      "brandId": "680e8bc08cdb14f10180db6a",
      "url": "https://example.com/solutions/network-observability",
      "depthOfUrl": 4,
      "title": "Kubernetes Network Observability",
      "markdown": "## Awesome website content...",
      "createdAt": "2025-07-11T10:13:26.966Z"
    },
    {
      "id": "6870e3c687c81925622df89f",
      "crawlId": "6870e36787c81925622df818",
      "brandId": "680e8bc08cdb14f10180db6a",
      "url": "https://example.com/about",
      "depthOfUrl": 2,
      "title": "About Our Company",
      "markdown": "## About page content...",
      "createdAt": "2025-07-11T10:13:27.123Z"
    }
  ]
}
```

## Complete Example

Here's a complete example that demonstrates the full workflow:

```javascript
import Olyptik from "olyptik";

const olyptik = new Olyptik({ apiKey: 'your-api-key' });

async function scrapeWebsite() {
  try {
    // Step 1: Create crawl
    const crawl = await olyptik.createCrawl({
      startUrl: 'https://example.com',
      maxResults: 50
    });
    console.log('Crawl created:', crawl.id);

    // Step 2: Wait for completion (polling approach)
    let isComplete = false;
    while (!isComplete) {
      const status = await olyptik.getCrawl(crawl.id);
      console.log('Status:', status.status);
      
      if (status.status === 'completed' || status.status === 'failed') {
        isComplete = true;
      } else {
        await new Promise(resolve => setTimeout(resolve, 30000)); // Wait 30 seconds
      }
    }

    // Step 3: Get results
    const results = await olyptik.getCrawlResults(crawl.id, 1, 50);
    console.log('Total pages scraped:', results.totalResults);
    
    return results;
  } catch (error) {
    console.error('Error:', error);
  }
}
```

## Next Steps

- Check out our [API Reference](/api-reference) for detailed endpoint documentation
- Learn about [Authentication](/essentials/authentication) and security best practices
- Explore [Advanced Usage](/essentials/advanced-usage) for more complex scenarios

## Need Help?


- Contact our [support team](mailto:support@olyptik.io) for assistance
- Join our [Discord community](https://discord.gg/olyptik) for discussions
- Check our [FAQ](/essentials/faq) for common questions